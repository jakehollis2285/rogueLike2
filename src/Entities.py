...
def initDefaultEntities(GRID_X, GRID_Y, GRID_Z):
    ''' entities are static for each level in the world change this '''
    player = Entity('@', GRID_Z, int(GRID_X / 2), int(GRID_Y / 2), tcod.white)
    npc = Entity('@', GRID_Z, int(GRID_X / 2), int(GRID_Y / 2) - 5, tcod.yellow)
    return [npc, player]
